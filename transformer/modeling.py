...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        #self.map = nn.Linear(12*length*length,4*length*length)
        #self.pool = nn.MaxPool1d(3, stride=3)
        #self.m2 = nn.MaxPool1d(6, stride=6)

    def forward(self, hidden_states, attention_mask):
        all_encoder_layers = []
        all_encoder_atts = []
        for _, layer_module in enumerate(self.layer):
            #new_hidden_states = self.m2(hidden_states)
            all_encoder_layers.append(hidden_states)
            hidden_states, layer_att = layer_module(
                hidden_states, attention_mask)
            #length = layer_att.size(2)
            #layer_att_reshape = layer_att.reshape(64, 12 * length * length)
            #map = nn.Linear(layer_att_reshape.size(1), 4 * length * length)
            #map.to('cuda')
            #new_layer_att = self.map(layer_att_reshape)
            #new_layer_att = new_layer_att.reshape(64, 4, length, length)
            #layer_att = layer_att.reshape(64, layer_att.size(2) * layer_att.size(3), 12)
            #new_layer_att = self.pool(layer_att)
            all_encoder_atts.append(layer_att)

        #new_hidden_states = self.m2(hidden_states)  # 768->128
        all_encoder_layers.append(hidden_states)
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.rep_linear = nn.Linear(768, 512)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        ##
        dim_encoded_layers = []
        for rep in encoded_layers:
            rep = self.rep_linear(rep)
            dim_encoded_layers.append(rep)
        return dim_encoded_layers, layer_atts, pooled_output
